...
AND CASE -- OFF CAMPUS HOUSING FLAG IN RORPRST
	WHEN (X.SARADAP_STYP_CODE in ('F', '1') and RORPRST_XHS_LOCK_IND = 'Y' and RORPRST_XHS = '3') THEN '1' -- off campus
	WHEN (X.SARADAP_STYP_CODE in ('F', '1') and RORPRST_XHS_LOCK_IND = 'Y' and RORPRST_XHS in ('1', '4')) THEN '3' -- family/relative
	WHEN (X.SARADAP_STYP_CODE not in ('F', '1') and RORPRST_XHS = '3') THEN '1' -- off campus
	WHEN (X.SARADAP_STYP_CODE not in ('F', '1') and RORPRST_XHS in ('1', '4')) THEN '3' -- family/relative
END = RORALGS_KEY_3
AND CASE -- level code

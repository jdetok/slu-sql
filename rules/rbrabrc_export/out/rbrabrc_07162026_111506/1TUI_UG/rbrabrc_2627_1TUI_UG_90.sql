...
FROM SARADAP X, SARAPPD b, RORALGS, STVAPDC, SARAATT
...
AND CASE 
    WHEN SARAATT_ATTS_CODE = 'PSMV' THEN 'PSMV'
...
AND SARAATT_PIDM = X.SARADAP_PIDM
AND SARAATT_TERM_CODE = X.SARADAP_TERM_CODE_ENTRY
AND RORALGS_AIDY_CODE = :AIDY
AND b.SARAPPD_PIDM = X.SARADAP_PIDM	
-- AND X.SARADAP_PIDM = :PIDM

;
select sgrsatt_atts_code, stvatts_desc
from sgrsatt
join stvatts on stvatts_code = sgrsatt_atts_code
where sgrsatt_atts_code like 'PS%'
group by sgrsatt_atts_code, stvatts_desc;
desc stvatts;
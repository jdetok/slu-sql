...
join rorstat on rorstat_pidm = tbraccd_pidm 
    ...
    and rorstat_tgrp_code not in ('WITHDR', 'NAT')
...
and tbraccd_detail_code not in ('RLAF', 'HCBF', 'HCBS', 'HFVL')
group by spriden_id, tbraccd_pidm, tbraccd_term_code, rorstat_pgrp_code, rprarsc_actual_amt
having sum(tbraccd_amount) > 0
;

select 
    spriden_id as bid, 
    rorprst_period as term, 
    rorstat_pgrp_code as pgrp, 
    rprarsc_actual_amt as rsrc,
    case when exists (
        select 1 from sfrstcr
        where sfrstcr_pidm = rorprst_pidm
        and sfrstcr_term_code = rorprst_period
        and not regexp_like(sfrstcr_rsts_code, '^D.|^W.')
        and sfrstcr_bill_hr > 0
    ) then 'Y' else 'N' end as regd
from rorprst
...
select * from sfrstcr
where sfrstcr_pidm = 1437763
and sfrstcr_term_code = '202710'
and not regexp_like(sfrstcr_rsts_code, '^D.|^W.')
and sfrstcr_bill_hr > 0;

select * from stvrsts where stvrsts_code = 'WW';
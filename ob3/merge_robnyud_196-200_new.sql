...
-- Written by SFS Justin DeKock 06/11/2026, updated 06/22/2026
...
select * from (
    with term as (
        ...
        and a.sgbstdn_term_code_eff = (
            ...
            where z.sgbstdn_term_code_eff <= '202700'
            and z.sgbstdn_pidm = a.sgbstdn_pidm
        )
        and a.sgbstdn_pidm = (select spriden_pidm from spriden where spriden_change_ind is null and spriden_id = '001424678')
    ), hours as (
        select distinct sfrstcr_pidm as pidm
        from sfrstcr
        where sfrstcr_term_code in ('202620', '202700')
        and sfrstcr_bill_hr > 0
        -- and sfrstcr_pidm = (select spriden_pidm from spriden where spriden_change_ind is null and spriden_id = '001424678')
    ), legacy_yn as (
    ...
    where (
        ...
        and exists (
            ...
            and rp.rpratrm_term_code between (
                ...
                -- and z.sgbstdn_degc_code_1 = a.degree_code
                where z.sgbstdn_pidm = 1432857
                and z.sgbstdn_levl_code = 'UG'
                and z.sgbstdn_degc_code_1 = 'BA'
        ...
        and exists (
            ...
            and rp.rpratrm_term_code between (
                ...
                from sgbstdn z
                where z.sgbstdn_pidm = a.pidm
                and z.sgbstdn_majr_code_1 = a.major_code
                and z.sgbstdn_degc_code_1 = a.degree_code
                and z.sgbstdn_stst_code in ('AS', 'IL')
            ) and (select t from term)
            and rb.rfrbase_fsrc_code = 'FEDR'
...
-- update set
    ...
    r.robnyud_value_201 = src.cipc_code
-- ;
;
desc stvmajr;

select robnyud_pidm from robnyud where robnyud_value_197 = 'Y';
...
and exists (
    ...
    where robnyud_pidm = a.pidm
    and robnyud_value_202 is not null
)
;